...
const AnimatedCounter = ({ value }: { value: string }) => {
  ...
  const displayCount = React.useMemo(() => {
    if (numericValue.toString().length > 3 && !value.includes('Cr') && !value.includes('M') && !value.includes('/hr')) {
        if (count >= 1000) {
            return (count / 1000).toLocaleString('en-IN', {minimumFractionDigits: 0, maximumFractionDigits: 1}) + 'K';
        }
    }
    if (value.includes('M')) {
         if(count >= 1) return (count/1).toFixed(0) + 'M';
...
export default function KeyMetricsDelivery() {
  return (
    <section className="py-12 sm:py-20 bg-secondary/30">
      <div className="container">
        <div className="max-w-3xl mx-auto text-center mb-12">
            <Button variant="outline" className="rounded-full mb-4 border-primary text-primary hover:bg-primary/10 hover:text-primary font-bold" disabled>Our Network</Button>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold font-headline">
                Join a Thriving Delivery Fleet
            </h2>
            <p className="mt-4 text-lg text-muted-foreground">
                Our delivery partners are the backbone of our service, growing every day.
            </p>
        </div>
